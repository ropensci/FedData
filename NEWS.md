# FedData (4.4.0)
- Tests that exercise live web services now skip — rather than fail — when the upstream service is unreachable, throttling, or returning server errors. Federal data services are occasionally unavailable from continuous-integration runners, which caused spurious test failures unrelated to the package.
