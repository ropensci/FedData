  ...
  for(w in wests){
    for(n in norths){
      if(!file.exists(paste(raw.dir,'/',res,'/n',n,'w',w,'.zip',sep=''))){
        if(url.exists(paste('http://tdds3.cr.usgs.gov/Ortho9/ned/ned_',res,'/grid/n',n,'w',w,'.zip',sep=''))){
          cat(paste('\nDownloading: http://tdds3.cr.usgs.gov/Ortho9/ned/ned_',res,'/grid/n',n,'w',w,'.zip\n',sep=''))
          flush.console()
          f = CFILE(paste(raw.dir,'/',res,'/n',n,'w',w,'.zip',sep=''), mode="wb")
          curlPerform(url = paste('http://tdds3.cr.usgs.gov/Ortho9/ned/ned_',res,'/grid/n',n,'w',w,'.zip',sep=''), writedata = f@ref)
          close(f)
        }else{
