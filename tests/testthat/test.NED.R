...
test_that("The NED tiles are available at the correct URL", {
  ...
  tileWesting <- "100"
  url <- paste0(
    "https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/", res, "/ArcGrid/USGS_NED_", res, "_n", tileNorthing, "w", tileWesting,
    "_ArcGrid.zip"
  )
...
test_that("NED URLs fall back to legacy format", {
  ...
  if (httr::http_error(url)) {
    url <- paste0(
      "https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/", res, "/ArcGrid/n", tileNorthing, "w", tileWesting,
      ".zip"
    )
  }
